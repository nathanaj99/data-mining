...
lines = lines.groupByKey().map(lambda x: set(x[1])).filter(lambda x: len(x) > threshold)
...
# -------- MAP REDUCE PHASE 1 ---------
mr1 = lines.mapPartitions(lambda x: apriori(list(x), candidate_items1, supportPart)).flatMap(lambda x: x)\
    .map(lambda x: (tuple(sorted(x)), 1)).groupByKey().map(lambda x: x[0]).collect()
